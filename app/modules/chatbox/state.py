...
from .providers.mock import MockIntentProvider
from .service import create_chatbox_store

_debug_stores_by_dataset = {}
_debug_provider = MockIntentProvider()
...
def get_debug_store_for_context(selection_context: SelectionContext):
    ...
    if selection_context.dataset_id not in _debug_stores_by_dataset:
        _debug_stores_by_dataset[selection_context.dataset_id] = create_chatbox_store(
            selection_context.dataset_id
        )
...
def reset_debug_store_for_context(selection_context: SelectionContext):
    ...
    _debug_provider.reset(selection_context.dataset_id)
    return _debug_stores_by_dataset[selection_context.dataset_id]


def get_debug_provider() -> MockIntentProvider:
