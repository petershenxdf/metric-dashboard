...
def _validate_inputs(
    ...
    if n_clusters > matrix.shape[0]:
        raise ValueError("n_clusters must not exceed the number of candidate points")
    if (
...
def _initial_centers(matrix: np.ndarray, n_clusters: int) -> np.ndarray:
    ...
    center_indices = [int(np.argmin(distances_to_mean))]
...
def _assign_labels(matrix: np.ndarray, centers: np.ndarray) -> np.ndarray:
    return np.argmin(_squared_distances_to_centers(matrix, centers), axis=1)
